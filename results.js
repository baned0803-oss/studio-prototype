...
function calculateTotalCost(rates, startMin, endMin, targetDayOfWeek) {
    ...
    // 例: 18:00〜20:30 → 3時間分の料金を計算 (端数切り上げ)
...
function renderCards(items, requestedPeople, requestedArea, searchMode, totalDuration, targetDayOfWeek, startTime, endTime) {
    ...
    items.forEach(item => {
        ...
        // 🎯 修正: 時間表示をsearchModeに応じて変更
        const timeRangeText = (searchMode === 'day' && startTime && endTime) 
            ? ` (${escapeHtml(startTime)} - ${escapeHtml(endTime)})` 
            : ''; // 深夜パック('night')の場合は時間を表示しない
        ...
        const costHtml = `
            <div class="cost-display">
                <div class="total-cost">
                    ...
                    <div class="price">${formatPrice(totalCost)}${timeRangeText}</div>
                </div>
